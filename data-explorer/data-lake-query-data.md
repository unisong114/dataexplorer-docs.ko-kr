...
1. 명령을 사용 `.create external table` 하 여 Azure 데이터 탐색기에서 외부 테이블을 만듭니다. , 및와 같은 추가 외부 테이블 명령은 `.show` `.drop` `.alter` [외부 테이블 명령](kusto/management/externaltables.md)에 설명 되어 있습니다.
...
예를 들어 테이블이 yyyy/MM/dd 형식의 DateTime 파티션으로 정의 된 경우 (기본값) URI 저장소 파일 경로는 *container1/yyyy/mm/dd/all_exported_blobs*이어야 합니다. 
    > * 외부 테이블이 datetime 열에 의해 분할 된 경우 쿼리에서 닫힌 범위에 대 한 시간 필터를 항상 포함 합니다. 예를 들어 쿼리는 `ArchivedProducts | where Timestamp between (ago(1h) .. 10m)` 이 (열린 범위) 1 보다 더 잘 수행 되어야 합니다 `ArchivedProducts | where Timestamp > ago(1h)` . 
    > * [지원 되](ingestion-supported-formats.md) 는 모든 수집 형식은 외부 테이블을 사용 하 여 쿼리할 수 있습니다.
...
Json 형식의 외부 테이블을 만들 수 있습니다. 자세한 내용은 [외부 테이블 명령](kusto/management/externaltables.md) 을 참조 하세요.

1. 명령을 사용 `.create external table` 하 여 *Externaltablejson*이라는 테이블을 만듭니다.
...
외부 테이블을 쿼리하려면 함수를 사용 하 `external_table()` 고 테이블 이름을 함수 인수로 제공 합니다. 쿼리의 나머지 부분은 표준 Kusto 쿼리 언어입니다.
...
동일한 쿼리 내에서 외부 테이블과 수집 데이터 테이블을 모두 쿼리할 수 있습니다. 사용자 [`join`](kusto/query/joinoperator.md) 또는 [`union`](kusto/query/unionoperator.md) 외부 테이블에는 Azure 데이터 탐색기, SQL server 또는 기타 원본의 추가 데이터가 포함 되어 있습니다. 를 사용 [`let( ) statement`](kusto/query/letstatement.md) 하 여 외부 테이블 참조에 약식 이름을 할당 합니다.
...
에 로그인 하 여 [https://dataexplorer.azure.com/clusters/help/databases/Samples](https://dataexplorer.azure.com/clusters/help/databases/Samples) *TaxiRides* 외부 테이블을 쿼리 합니다. 
...
원격 저장소에서 인출 되는 데이터의 양을 줄이려면 압축을 사용 합니다. Parquet format의 경우 열 그룹을 개별적으로 압축 하는 내부 Parquet 압축 메커니즘을 사용 합니다. 따라서 별도로 읽을 수 있습니다. 압축 메커니즘 사용의 유효성을 검사 하려면 파일 이름이 "release.tar.gz" 또는 ". <filename> parquet"와 반대 되는 ". parquet" 또는 " <filename> . snappy"와 같이 지정 되었는지 확인 <filename> 합니다. 
