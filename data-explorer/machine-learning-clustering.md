...
ms.locfileid: "83374044"
---
# <a name="machine-learning-capability-in-azure-data-explorer"></a>Azure 데이터 탐색기의 기계 학습 기능

빅 데이터 분석 플랫폼인 Azure 데이터 탐색기는 기본 제공 [변칙 검색 및 예측](anomaly-detection.md) 함수를 사용 하 여 비정상적인 동작에 대 한 서비스 상태, QoS 또는 작동 하지 않는 장치를 모니터링 하는 데 사용 됩니다. 비정상 패턴을 감지 하면 변칙을 완화 하거나 해결 하기 위해 RCA (근본 원인 분석)가 수행 됩니다.

진단 프로세스는 복잡 하 고 길고 도메인 전문가에 의해 수행 됩니다. 이 프로세스에는 동일한 시간 프레임에 대 한 여러 원본의 추가 데이터를 가져오고 조인 하는 작업이 포함 되어 있습니다 .이를 통해 여러 차원에 대 한 값 분포를 찾고 추가 변수를 차트로 작성 하 고 도메인 지식 및 intuition을 기반으로 하는 기타 기술을 사용할 수가 있습니다. 이러한 진단 시나리오는 Azure 데이터 탐색기에서 일반적 이므로 진단 단계를 더 쉽게 수행 하 고 RCA의 지속 시간을 단축 하기 위해 machine learning 플러그 인을 사용할 수 있습니다.

Azure 데이터 탐색기에는 [`autocluster`](kusto/query/autoclusterplugin.md) , 및의 세 가지 Machine Learning 플러그 인이 있습니다. [`basket`](kusto/query/basketplugin.md) [`diffpatterns`](kusto/query/diffpatternsplugin.md) 모든 플러그 인은 클러스터링 알고리즘을 구현 합니다. 및 플러그 인은 `autocluster` `basket` 단일 레코드 집합을 클러스터 하 고 `diffpatterns` 플러그 인은 두 레코드 집합 간의 차이점을 클러스터링 합니다.
...
서비스 예외 수는 전체 서비스 트래픽과 관련이 있습니다. 매일 월요일에서 금요일 까지의 일별 패턴을 명확 하 게 볼 수 있으며, 중간 규모의 서비스 예외 수를 계산 하 고, 밤에는 개수를 감소 시킬 수 있습니다. 플랫 낮은 수가 주말에 표시 됩니다. Azure 데이터 탐색기에서 [시계열 변칙 검색](anomaly-detection.md#time-series-anomaly-detection) 을 사용 하 여 예외 급증을 감지할 수 있습니다.
...
천 개 미만의 예외가 있지만 각 열에 여러 값이 있으므로 일반적인 세그먼트를 찾는 것은 여전히 어렵습니다. [`autocluster()`](kusto/query/autoclusterplugin.md)다음 쿼리에서 볼 수 있는 것 처럼 플러그 인을 사용 하 여 일반 세그먼트의 작은 목록을 즉시 추출 하 고 스파이크의 2 분 내에 관심 있는 클러스터를 찾을 수 있습니다.
...
[`diffpatterns()`](kusto/query/diffpatternsplugin.md)플러그 인은 및의 제한을 극복 합니다 `autocluster` `basket` . `Diffpatterns`두 레코드 집합을 사용 하 여 서로 다른 주 세그먼트를 추출 합니다. 한 집합에는 일반적으로 조사 중인 비정상 레코드 집합 (및에서 분석 한 집합)이 포함 `autocluster` `basket` 됩니다. 다른 집합에는 참조 레코드 집합 (기준선)이 포함 됩니다. 

아래 쿼리에서는를 사용 하 여 `diffpatterns` 스파이크의 2 분 내에 흥미로운 클러스터를 찾습니다 .이는 기준선 내의 클러스터와 다릅니다. 기본 기간은 15:00 이전 8 분 (스파이크가 시작 된 경우)으로 정의 됩니다. 또한 특정 레코드가 기준 또는 비정상 집합에 속하는지 여부를 지정 하는 AB (이진 열)로 확장 해야 합니다. `Diffpatterns`두 클래스 레이블이 비정상 및 기준 플래그 (AB)에서 생성 된 감독 학습 알고리즘을 구현 합니다.
...
가장 기준 세그먼트는로 추출 된 세그먼트와 동일 합니다 `autocluster` . 2 분 비정상 창에서의 적용 범위는 65.74% 이기도 합니다. 그러나 8 분 기준 기간의 적용 범위는 1.7%입니다. 차이점은 64.04%입니다. 이러한 차이는 비정상 스파이크와 관련 된 것으로 보입니다. 아래 쿼리에 표시 된 것 처럼 원본 차트를이 문제가 있는 세그먼트에 속하는 레코드 및 다른 세그먼트로 분할 하 여이 가정을 확인할 수 있습니다.
...
Azure 데이터 탐색기 Machine Learning 플러그 인은 많은 시나리오에서 유용 합니다. `autocluster`및는 `basket` 자율 학습 알고리즘을 구현 하 고 사용 하기 쉽습니다. `Diffpatterns`는 감독 된 학습 알고리즘을 구현 하 고 더 복잡 하지만 RCA에 대 한 차별화 세그먼트를 추출 하는 것이 더 강력 합니다.
