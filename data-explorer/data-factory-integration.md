...
복사 작업을 사용 하 여 데이터를 Azure 데이터 탐색기로 복사 하거나 [쿼리](kusto/management/data-ingestion/ingest-from-query.md) ( `.set-or-append` ,, `.set-or-replace` `.set` , `.replace)` 및 [저장소에서 수집](kusto/management/data-ingestion/ingest-from-storage.md) `.ingest` ) ()에서 수집 하는 등의 수집 명령을 사용 하 여 데이터를 복사할 수 있습니다. 
...
Azure 데이터 탐색기가 원본인 경우 쿼리를 포함 하는 조회, 복사 또는 명령 작업을 사용 하는 경우 성능 정보에 대 한 [쿼리 모범 사례](kusto/query/best-practices.md) 및 [복사 작업을 위한 ADF 설명서](/azure/data-factory/copy-activity-performance)를 참조 하세요.
