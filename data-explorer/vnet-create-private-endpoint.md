...
1. **아웃 바운드 설정** 창에서 다음 필드를 입력 합니다.
    ...
    |**설정** | **제안 값** | **필드 설명**
    |---|---|---|
    | Load Balancer | 엔진 또는 *데이터 관리* Load Balancer | 클러스터 엔진에 대해 만든 부하 분산 장치를 선택 합니다. 외부 또는 내부 부하 분산 장치를 사용할 수 있습니다.  부하 분산 장치 엔진 이름의 형식은 다음과 같습니다. <br> *kucompute-{clustername}-elb (* 외부 부하 분산 장치) <br> *kucompute-{clustername}-* 내부 부하 분산 장치에 대 한 ilb<br> *부하 분산 장치 데이터 관리 이름은 다음 형식으로 지정 됩니다. <br> kudatamgmt-{clustername}-elb <br> kudatamgmt-{clustername}-ilb*|
