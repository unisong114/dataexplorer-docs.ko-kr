---
title: Azure 데이터 탐색기에서 데이터를 쿼리 하는 Power Apps 응용 프로그램 만들기
...
* [Power Apps 제품군](https://docs.microsoft.com/powerapps/powerapps-overview)에 대해 잘 알고 있어야 합니다.
...
1. 검색 창에서 **Azure 데이터 탐색기** 를 검색 합니다. 결과 옵션에서 **Azure 데이터 탐색기** 를 선택 합니다.
    ...
    :::image type="content" source="media/power-apps-connector/search-adx.png" alt-text="Power Apps에서 새 연결 만들기":::

1. **Azure 데이터 탐색기** Popup에서 **만들기** 를 선택 합니다. 필요에 따라 자격 증명을 제공 합니다.

    :::image type="content" source="media/power-apps-connector/create-connector.png" alt-text="Power Apps에서 새 연결 만들기":::
...
1. 결과 드롭다운에서 **캔버스** 를 선택 합니다.
    ...
    :::image type="content" source="media/power-apps-connector/create-new-app.png" alt-text="Power Apps에서 새 연결 만들기":::

1. 비어 있는 **앱** 섹션에서 **태블릿 레이아웃**을 선택 합니다.

    :::image type="content" source="media/power-apps-connector/blank-canvas.png" alt-text="Power Apps에서 새 연결 만들기":::
...
1. 결과 옵션에서 **Azure 데이터 탐색기** 를 선택 합니다.
    ...
    :::image type="content" source="media/power-apps-connector/data-connectors-adx.png" alt-text="Power Apps에서 새 연결 만들기":::

이제 **Azure 데이터 탐색기** 포함 된 **앱에서** 라는 새 영역이 표시 됩니다.

   :::image type="content" source="media/power-apps-connector/adx-appears.png" alt-text="Power Apps에서 새 연결 만들기":::
...
1. 왼쪽 탐색에서 **저장** 을 선택 합니다.
    ...
    :::image type="content" source="media/power-apps-connector/save-app.png" alt-text="Power Apps에서 새 연결 만들기":::
...
1. 결과 옵션에서 **동적 스키마** 를 선택 합니다. 이 기능을 사용 하도록 설정 합니다.
    ...
    :::image type="content" source="media/power-apps-connector/dynamic-schema.png" alt-text="Power Apps에서 새 연결 만들기":::

1. **비 위임 가능한 쿼리 설정의 데이터 행 제한을** 검색 합니다. 반환 된 레코드 제한을 설정 합니다.

    :::image type="content" source="media/power-apps-connector/set-limit.png" alt-text="Power Apps에서 새 연결 만들기":::
...
1. **항목** 입력 상자를 ["캘리포니아", "MICHIGAN"]로 채웁니다.
    ...
    :::image type="content" source="media/power-apps-connector/populate-dropdown.png" alt-text="Power Apps에서 새 연결 만들기" lightbox="media/power-apps-connector/populate-dropdown.png":::
...
1. **스키마 캡처** 단추를 클릭 합니다. 처리 시간을 허용 합니다.
    ...
    :::image type="content" source="media/power-apps-connector/capture-schema.png" alt-text="Power Apps에서 새 연결 만들기":::
...
1. 결과 팝아웃에서 **+ 필드 추가** 를 선택 합니다. 
    ...
    :::image type="content" source="media/power-apps-connector/insert-data-table-small.png" alt-text="Power Apps에서 새 연결 만들기" lightbox="media/power-apps-connector/insert-data-table.png":::

1. 원하는 필드를 선택 하 고 **추가** 단추를 클릭 합니다. 선택한 데이터 테이블의 미리 보기가 표시 됩니다.

    :::image type="content" source="media/power-apps-connector/preview-table.png" alt-text="Power Apps에서 새 연결 만들기":::
...
1. 드롭다운을 시도 하 고 데이터 테이블을 스크롤하여 성공적으로 데이터 검색 및 프레젠테이션을 확인 합니다.
    ...
    :::image type="content" source="media/power-apps-connector/preview-app.png" alt-text="Power Apps에서 새 연결 만들기":::

### <a name="limitations"></a>제한 사항

* Power Apps는 클라이언트에 반환 되는 결과 레코드를 최대 2000 개로 제한 합니다. 이러한 레코드에 대 한 전체 메모리는 64 MB를 초과할 수 없으며 실행 하는 데 7 분이 소요 됩니다.
* 커넥터는 [포크](https://docs.microsoft.com/azure/data-explorer/kusto/query/forkoperator) 및 [패싯](https://docs.microsoft.com/azure/data-explorer/kusto/query/facetoperator) 연산자를 지원 하지 않습니다.
* **시간 제한 예외**: 커넥터에는 7 분의 제한 시간 제한이 있습니다. 잠재적 시간 제한 문제를 방지 하려면 쿼리를 더 효율적으로 실행 하 여 더 빠르게 실행 하거나 청크로 분리 합니다. 각 청크는 쿼리의 다른 부분에서 실행할 수 있습니다. 자세한 내용은 [쿼리 모범 사례](https://docs.microsoft.com/azure/data-explorer/kusto/query/best-practices)를 참조 하세요.
