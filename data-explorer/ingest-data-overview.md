...
* Azure의 분석 워크 로드를 위한 완전히 관리 되는 데이터 통합 서비스인 ADF (Azure Data Factory)는 [지원 되는 데이터 저장소와 형식을](/azure/data-factory/copy-activity-overview#supported-data-stores-and-formats)사용 하 여 azure 데이터 탐색기 간에 데이터를 복사 하는 데 사용 됩니다. 자세한 내용은 [Azure Data Factory에서 Azure 데이터 탐색기로 데이터 복사](data-factory-load-data.md)를 참조 하세요.
...
* Azure 데이터 탐색기 데이터 관리 서비스를 통해 데이터 수집(높은 처리량 및 안정적인 수집):
    ...
    [**수집 일괄 처리**](kusto/api/netfx/kusto-ingest-queued-ingest-sample.md)(SDK에서 제공): 클라이언트는 Azure Blob Storage(Azure 데이터 탐색기 데이터 관리 서비스에서 지정)에 데이터를 업로드하고 Azure 큐에 알림을 게시합니다. 수집 일괄 처리는 안정적이고 저렴한 대용량 데이터 수집을 위해 권장되는 기술입니다.
...
* 수집된 데이터의 효과적인 보존 정책은 데이터베이스의 보존 정책에서 파생됩니다. 자세한 내용은 [보존 정책](kusto/management/retentionpolicy.md)을 참조하세요. 데이터를 수집하려면 **테이블 수집기** 또는 **데이터베이스 수집기** 권한이 필요합니다.
...
* [CSV 매핑](kusto/management/mappings.md#csv-mapping)(선택 사항)은 모든 서수 기반 형식에서 작동합니다. 수집 명령 매개 변수를 사용하여 수행하거나 [테이블에 미리 만들고](kusto/management/create-ingestion-mapping-command.md) 수집 명령 매개 변수에서 참조할 수 있습니다.
* [JSON 매핑](kusto/management/mappings.md#json-mapping)(필수) 및 [Avro 매핑](kusto/management/mappings.md#avro-mapping)(필수)은 ingest 명령 매개 변수를 사용하여 수행할 수 있습니다. [테이블에 미리 만들고](kusto/management/create-ingestion-mapping-command.md) ingest 명령 매개 변수에서 참조할 수도 있습니다.
