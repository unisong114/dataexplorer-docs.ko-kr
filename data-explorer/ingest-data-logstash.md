...
ms.locfileid: "96303358"
---
# <a name="ingest-data-from-logstash-to-azure-data-explorer-preview"></a>Logstash 태에서 Azure 데이터 탐색기로 데이터 수집 (미리 보기)

[Logstash](https://www.elastic.co/products/logstash)는 동시에 여러 원본의 데이터를 수집하고, 데이터를 변환한 다음, 데이터를 즐겨찾는 “스태시”에 전송하는 오픈 소스인 서버 쪽 데이터 처리 파이프라인입니다. 이 문서에서는 로그 및 원격 분석 데이터에 대 한 빠르고 확장성이 뛰어난 데이터 탐색 서비스인 Azure 데이터 탐색기로 해당 데이터를 보냅니다. 처음에는 테스트 클러스터에서 테이블 및 데이터 매핑을 만든 다음 Logstash 태 트를 통해 테이블에 데이터를 보내고 결과의 유효성을 검사 합니다.

> [!NOTE]
> 이 커넥터는 현재 json 데이터 형식만 지원 합니다.

## <a name="prerequisites"></a>사전 요구 사항

* Azure 구독 계정이 없는 경우 시작 하기 전에 [무료 Azure 계정](https://azure.microsoft.com/free/) 을 만듭니다.
...
| **매핑으로** | 매핑은 들어오는 이벤트 json 문자열을 올바른 행 형식으로 매핑하는 데 사용됩니다(어떤 속성이 열에 들어가는지 정의). |
