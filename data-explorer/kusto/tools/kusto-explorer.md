...
* 대신에 브라우저를 사용 하 여 Kusto 클러스터에 액세스 합니다.`https://<your_cluster>.kusto.windows.net.`
...
|[변칙 차트](#anomaly-chart)|    시간 차트와 유사 하지만 기계 학습 이상 알고리즘을 사용 하 여 시계열 데이터의 변칙을 찾습니다. 변칙 검색의 경우, Kusto 탐색기는 [series_decompose_anomalies](../query/series-decompose-anomaliesfunction.md) 함수를 사용 합니다.
...
> [!NOTE]
> <a id="anomaly-chart">변칙 차트</a>: 알고리즘은 다음 두 열로 구성 된 시계열 데이터를 필요로 합니다.
>* 고정 간격 버킷의 시간
>* 변칙 검색을 위한 숫자 값으로, Kusto. 탐색기에서 시계열 데이터를 생성 하 고, 시간 필드를 기준으로 요약 하 고, 시간 버킷 bin을 지정 합니다.
...
> 데이터 보기 설정:
>
> Kusto 탐색기는 고유 열 집합 마다 사용 되는 설정을 추적 합니다. 열을 다시 정렬 하거나 제거 하면 데이터 뷰가 저장 되 고 동일한 열이 있는 데이터가 검색 될 때마다 다시 사용 됩니다. 뷰를 기본값으로 다시 설정 하려면 **보기** 탭에서 **보기 다시 설정**을 선택 합니다. 
...
||---------*데이터 뷰*---------|
|보기 다시 설정| [데이터 뷰 설정을](#dvs) 기본값으로 다시 설정 합니다. |
|열 값 탐색|열 값 분포 표시|
