---
title: series_moving_avg_lf ()-Azure 데이터 탐색기
description: 이 문서에서는 Azure 데이터 탐색기에서 series_moving_avg_lf () 사용자 정의 함수에 대해 설명 합니다.
author: orspod
ms.author: orspodek
...
ms.locfileid: "89614830"
---
# <a name="series_moving_avg_lf"></a>series_moving_avg_lf ()

계열에 이동 평균 필터를 적용 합니다.

함수는 `series_moving_avg_lf()` 동적 숫자 배열을 입력으로 포함 하는 식을 사용 하 여 [단순 이동 평균](https://en.wikipedia.org/wiki/Moving_average#Simple_moving_average) 필터를 적용 합니다.
...
`series_moving_avg_lf(`*y_series* `,` *n* `, [` *가운데*`])`
...
`series_moving_avg_lf()` 는 사용자 정의 함수입니다. 쿼리에 해당 코드를 포함 하거나 데이터베이스에 설치할 수 있습니다. Ad hoc 및 영구 사용의 두 가지 사용 옵션이 있습니다. 예제는 아래 탭을 참조 하세요.
...
:::image type="content" source="images/series-moving-avg-lf/moving-average-five-bins.png" alt-text="5 개 계급의 이동 평균을 보여 주는 그래프" border="false":::
