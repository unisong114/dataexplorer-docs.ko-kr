...
테이블의 형식화 된 열에 해시 파티션 키를 적용 하는 `string` 것은 *대부분* 의 쿼리에서 `==` `in()` `string` *큰 차원* (10M 이상의 카디널리티)의 특정 형식 열에 대해 같음 필터 (,) 및/또는 집계/조인을 사용 `application_ID` `tenant_ID` 하는 경우에 적합 합니다 `user_ID` .
...
* [순서 섞기 전략](../query/shufflequery.md)을 사용 하 고 `shuffle key` 에서 사용 되는 쿼리는 `join` `summarize` `make-series` 클러스터 노드 간에 이동 하는 데 필요한 데이터 양이 크게 감소 하기 때문에 더 나은 성능을 기대할 수 있습니다.
...
다음은 라는 형식화 된 열에 대 한 해시 파티션 키입니다 `string` `tenant_id` .
...
테이블에서 형식화 된 열에 균일 한 범위 datetime 파티션 키를 적용 하는 것 `datetime` 은 테이블의 데이터 수집이 열 *unlikely* 에 따라 정렬 되지 않는 경우에 적합 합니다. 이러한 경우 제한 된 시간 범위의 레코드를 포함 하 여 각 익스텐트가 종료 되도록 익스텐트 간에 데이터 순서를 다시 조정 하는 것이 도움이 될 수 있습니다 `datetime` . 그러면 쿼리 시간에 열에 대 한 필터가 더 효율적으로 생성 됩니다.
...
* `RangeSize``timespan`각 datetime 파티션의 크기를 나타내는 스칼라 상수입니다.
  * 시작 하는 데 권장 되는 값은 `1.00:00:00` (1 일)입니다.
  * 값을 설정 하는 것은 사용 *하지* 않는 것이 좋습니다 .이 값은 테이블에 작은 익스텐트가 많이 있지만 함께 병합할 수 없기 때문입니다.
* `Reference`정렬 된 `datetime` datetime 파티션에 따라 고정 된 특정 시점을 나타내는 형식의 스칼라 상수입니다.
  * 시작 하는 데 권장 되는 값은 `1970-01-01 00:00:00` 입니다.
  * Datetime 파티션 키에 값이 있는 레코드가 있는 경우 `null` 해당 파티션 값은의 값으로 설정 됩니다 `Reference` .

#### <a name="example"></a>예제

다음은 라는 형식화 된 열에 대 한 균일 한 datetime 범위 파티션 키입니다. `datetime``timestamp`
...
* **파티션 키**:
  ...
  * 테이블에는 `2` 다음 3 가지 옵션 중 하나를 사용 하 여 최대 파티션 키가 있을 수 있습니다.
...
2. 이라는 형식화 된 열에 대 한 균일 한 datetime 범위 파티션 키 `datetime``timestamp`
    - `datetime(1970-01-01)`각 파티션에 대해 크기를 사용 하 여를 참조 지점으로 사용 `1d` 합니다.
...
* 해시 파티션 키에 다른 값 보다 훨씬 더 많은 값이 포함 된 경우 (예: 빈 문자열, 일반 값 (예: `null` 또는 `N/A` ) 또는 데이터 집합에서 더 많이 발생 하는 엔터티 (예:)를 나타내는 경우 `tenant_id` , 클러스터의 노드 간에 분산 된 데이터 배포에 영향을 미칠 수 있으며 쿼리 성능이 저하 될 수 있습니다.
* 균일 범위의 datetime 파티션 키에 열에 포함 된 대부분의 값 (예: 과거 또는 미래의 시간 값)에서 "far"로 충분 한 비율의 값이 있는 경우 데이터 분할 프로세스의 오버 헤드가 증가 하 고 클러스터에서 추적을 유지 해야 하는 작은 익스텐트가 늘어날 수 있습니다.
