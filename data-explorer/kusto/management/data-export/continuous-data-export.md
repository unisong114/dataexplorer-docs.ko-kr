...
* 각 연속 내보내기 반복에서 내보낸 파일 수는 외부 테이블이 분할 되는 방식에 따라 달라 집니다. 자세한 내용은 [외부 테이블로 내보내기 명령](export-data-to-an-external-table.md)에서 참고 섹션을 참조 하세요. 각 연속 내보내기 반복은 항상 *새* 파일에 쓰고 기존 파일에 추가 하지 않습니다. 결과적으로, 내보낸 파일의 수도 연속 내보내기가 실행 되는 빈도 (매개 변수)에 따라 달라 집니다 `intervalBetweenRuns` .
