...
쿼리 매개 변수 `hint.strategy = shuffle` 또는 `hint.shufflekey = <key>`를 사용 하 여 순서 섞기 쿼리 전략을 설정 합니다.

테이블에 [데이터 분할 정책을](../management/partitioningpolicy.md) 정의 합니다. 

클러스터 `shufflekey` 노드 간에 이동 하는 데 필요한 데이터 양이 줄어들기 때문에 더 나은 성능을 위해 테이블의 해시 파티션 키로 설정 합니다.
...
복합 키가 너무 고유 하지만 각 키가 충분 하지 않은 경우이 `hint` 키를 사용 하 여 섞은 연산자의 모든 키로 데이터 순서를 섞기 합니다.
섞은 연산자에 또는 `summarize` `join`와 같은 다른 순서 섞기 연산자가 있는 경우에는 쿼리가 좀 더 복잡 해지고 힌트를 적용 합니다. 전략 = 순서 섞기는 적용 되지 않습니다.
...
쿼리 계획 중에 `hint.strategy=shuffle` 전략을 무시 하는 대신를 적용 하 여 복합 키 [`ActivityId`, `numeric_column`]를 기준으로 데이터 순서를 변경한 경우 결과가 올바르지 않습니다.
`summarize` 연산자가 `join` 연산자의 왼쪽에 있습니다. 이 연산자는 `join` 키의 하위 집합을 기준으로 그룹화 됩니다 .이 경우는 `ActivityId`입니다. 따라서는 `summarize` 키 `ActivityId`를 기준으로 그룹화 되는 반면 데이터는 복합 키 [`ActivityId`, `numeric_column`]로 분할 됩니다.
복합 키 [`ActivityId`, `numeric_column`]로 순서 섞기 키 `ActivityId` 에 대 한 순서 섞기가 유효 하 고 결과가 올바르지 않을 수 있습니다.
...
두 레코드에 대 한 복합 키가 다른 파티션 (56 및 65)에 매핑 되었지만 이러한 두 레코드의 `ActivityId`값은 동일 합니다. 의 `summarize` 왼쪽에 있는 연산자는 동일한 파티션에 `join` 있는 열의 `ActivityId` 유사한 값을 예상 합니다. 이 쿼리는 잘못 된 결과를 생성 합니다.

를 사용 `hint.shufflekey` 하 여에 `hint.shufflekey = ActivityId`대 한 조인에서 순서 섞기 키를 지정 하면이 문제를 해결할 수 있습니다. 이 키는 모든 무작위 재생 가능 연산자에 공통적입니다.
이 경우와 `join` `summarize` 는 모두 동일한 키로 순서를 순서 섞기 때문에 안전 하지 않습니다. 따라서 유사한 모든 값이 동일한 파티션에 있고 결과가 올바릅니다.
...
를 사용 하지 않고 `hint.num_partitions` 쿼리를 실행 하면 두 개의 파티션 (클러스터 노드 번호)만 사용 하 고 다음 쿼리는 ~ 1:10 분이 소요 됩니다.
...
왼쪽 테이블에는 `join` 키의 카디널리티가 ~ 14M 인 15M 레코드가 있습니다. 의 `join` 오른쪽은 150m 레코드를 사용 하 고 `join` 키의 카디널리티는 10M입니다.
...
순서 섞기 `join` 전략을 사용 하는 동안 쿼리는 ~ 4 초 후에 종료 되 고 메모리 사용량의 최대 사용량은 0.3입니다.
...
의 좌 변에 150M `join` 이 고 키의 카디널리티가 148M 인 큰 데이터 집합에 대해 동일한 쿼리를 시도 합니다. 의 오른쪽은 1.5 B `join` 이 고 키의 카디널리티는 ~ 100M입니다.

기본 `join` 전략을 사용 하는 쿼리는 4 분 후에 Kusto 제한 및 시간 제한에 도달 합니다.
순서 섞기 `join` 전략을 사용 하는 동안 쿼리는 ~ 34 초 후에 종료 되 고 메모리 사용량의 최대 사용량은 1.23 GB입니다.


다음 예에서는 두 개의 클러스터 노드가 있고, 테이블에 60M 레코드가 있고, `join` 키의 카디널리티가 2M 클러스터에 대 한 향상 된 기능을 보여 줍니다.
를 사용 하지 않고 `hint.num_partitions` 쿼리를 실행 하면 두 개의 파티션 (클러스터 노드 번호)만 사용 하 고 다음 쿼리는 ~ 1:10 분이 소요 됩니다.
