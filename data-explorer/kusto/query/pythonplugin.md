...
*T* `|` `single``,` *script* *output_schema* `,` *external_artifacts*[`hint.distribution` (`per_node`)] `python(`output_schema 스크립트 [`,` *script_parameters*] [external_artifacts] |  `evaluate` `=``)`

## <a name="arguments"></a>인수

* *output_schema*: Python `type` 코드에서 반환 하는 테이블 형식 데이터의 출력 스키마를 정의 하는 리터럴입니다.
...
* *script*: 실행할 `string` 유효한 Python 스크립트에 해당 하는 리터럴입니다.
* *script_parameters*: Python 스크립트 `dynamic` 에 예약 `kargs` 된 사전으로 전달 되는 이름/값 쌍의 속성 모음 인 선택적 리터럴입니다 ( [예약 된 Python 변수](#reserved-python-variables)참조).
...
* *external_artifacts*: 클라우드 저장소 `dynamic` 에서 액세스할 수 있고 런타임에 스크립트에서 사용할 수 있도록 설정할 수 있는 아티팩트에 대 한 이름 & URL 쌍의 속성 모음 인 선택적 리터럴입니다.
  ...
  * 아티팩트는 스크립트가 로컬 임시 디렉터리에서 사용할 수 있게 되 고 `.\Temp`, 속성 모음에 제공 된 이름이 로컬 파일 이름으로 사용 됩니다 (아래 [예제](#examples) 참조).
...
* `result`: Python `pandas` 스크립트에서 만든 데이터 프레임으로, 해당 값이 플러그 인을 따르는 Kusto 쿼리 연산자로 전송 되는 테이블 형식 데이터가 됩니다.
...
* [클러스터의 **구성** 탭에 있는 Azure Portal](https://docs.microsoft.com/azure/data-explorer/language-extensions)에서 플러그 인을 사용 하거나 사용 하지 않도록 설정 합니다.
...
* Python 이미지 `tensorflow`에는, `keras`, `torch`, `hdbscan` `xgboost` 및 기타 유용한 패키지와 같은 일반적인 ML 패키지도 포함 되어 있습니다.
* 플러그 인은 *numpy* 기본적으로 numpy `np`(as *pandas* ) & pandas `pd`를 가져옵니다.  필요에 따라 다른 모듈을 가져올 수 있습니다.
* **[쿼리](../management/data-ingestion/ingest-from-query.md) 및 [업데이트 정책](../management/updatepolicy.md) 에서 수집**
  ...
  * 원본 테이블이 [스트리밍](https://docs.microsoft.com/azure/data-explorer/ingest-data-streaming)수집을 사용 하 여 수집 업데이트 정책의 일부로 정의 된 쿼리에는 플러그 인을 사용할 수 *없습니다* .
...
* 에서 `Kusto.Explorer`python 스크립트를 포함 하는 여러 줄 문자열을 생성 하려면 즐겨 사용 하는 python 편집기 (*Jupyter*, *Visual Studio Code*, *PyCharm*등)에서 python 스크립트를 복사 하 고 다음 중 하나를 수행 합니다.
...
1. 일회성 필수 구성 요소:
  ...
  b. 해당 위치에 대 한 액세스를 허용 하도록 클러스터의 [콜아웃 정책을](../management/calloutpolicy.md) 변경 합니다.
    ...
    * 예를 들어에 `https://artifcatswestus.blob.core.windows.net/python`있는 blob에 대 한 액세스를 사용 하도록 설정 하려면 다음 명령을 실행 합니다.
...
3. 필요한 패키지 및 해당 종속성을 포함 하는 zip 파일을 만듭니다.
    ...
    * 메모:
        * 부모 폴더가 `.whl` *아니라* 파일 자체를 압축 해야 합니다.
        * 기본 샌드박스 이미지 `.whl` 에서 동일한 버전을 사용 하 여 이미 존재 하는 패키지에 대 한 파일을 건너뛸 수 있습니다.
    * 개인 패키지의 경우: zip 패키지 및 해당 종속성의 폴더

4. 1 단계에서 가져온 아티팩트 위치의 blob에 압축 된 파일을 업로드 합니다.

5. `python` 플러그 인 호출:
    * 이름 및 `external_artifacts` zip 파일 (blob URL)에 대 한 참조의 속성 모음을 사용 하 여 매개 변수를 지정 합니다.
    * 인라인 python 코드에서:에서 `Zipackage` `sandbox_utils` 가져오고 zip 파일의 이름을 `install()` 사용 하 여 해당 메서드를 호출 합니다.
