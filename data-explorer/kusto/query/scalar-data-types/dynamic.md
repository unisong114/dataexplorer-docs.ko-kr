...
편의상, `dynamic` 쿼리 텍스트에 나타나는 리터럴은 다른 Kusto 리터럴 (예: `datetime` 리터럴, 리터럴 등)을 포함할 수도 있습니다 `timespan` . JSON을 통한이 확장은 문자열을 구문 분석할 때 사용할 수 없지만 (예: 함수를 사용 하는 경우 또는 수집 데이터를 사용 하는 경우 `parse_json` )이 작업을 수행할 수 있습니다.
